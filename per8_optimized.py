...
# When a track disappears without crossing, auto-log OUT after this many frames
MISSING_TRACK_EXIT_FRAMES = BYTETRACK_LOST_BUFFER + 2
...
# Tracking heuristics
MIN_HISTORY_FOR_CAPTURE  = 4   # Require this many history points before fallback capture
...
def log_terminal(event_ts, direction, slot_str, region_class, suffix, kana, city, raw_text, suppressed=False):
    ...
    print(f"[{event_ts}] {direction.upper():3s}{tag}  slot={slot_str:>6s}  region_class='{rc_disp}'  suffix='{suf_disp}'  kana='{kana_disp}'  city='{city_disp}'  raw=\"{raw_text}\"")
...
def main():
    ...
    ever_outside = defaultdict(bool)   # Track whether we have seen this ID outside the gate yet
    capture_reason = {}
    ...
    def lookup_slot_for_plate(oid, plate_key_val, suffix_val):
        if oid in active_id_to_slot:
            return active_id_to_slot[oid]
        if plate_key_val and plate_key_val in active_plate_to_slot:
            return active_plate_to_slot[plate_key_val]
        if suffix_val and suffix_val in active_suffix_to_slot:
            return active_suffix_to_slot[suffix_val]
        return None

    def finalize_exit_event(oid, slot_idx, plate_key_val, suffix_val,
                            chosen_text, region_class, suffix, kana, city, engine_size, ts, det):
        if slot_idx is None:
            return False
        object_id = f"{slot_idx:02d}"
        allowed_out = suffix_event_allowed(suffix_event_history, suffix, "out", frame_idx, event_gap_frames)
        if allowed_out:
            write_row_flush(csv_writer, csv_file,
                            [ts, object_id, "car", "out", city or "", engine_size or "", kana or "", suffix or ""])
        slot_pool.release(slot_idx)
        last_event_f[oid] = frame_idx
        if DRAW_BBOXES and det is not None:
        ...
        log_terminal(ts, "out", f"{object_id}({suffix})" if suffix else object_id,
                     region_class, suffix, kana, city, chosen_text,
                     suppressed=not allowed_out)

        if suffix_val and active_suffix_to_slot.get(suffix_val) == slot_idx:
            active_suffix_to_slot.pop(suffix_val, None)
        if plate_key_val and active_plate_to_slot.get(plate_key_val) == slot_idx:
            active_plate_to_slot.pop(plate_key_val, None)

        owners = [owner for owner, slot in list(active_id_to_slot.items()) if slot == slot_idx]
        if oid not in owners:
            owners.append(oid)
        for owner in owners:
            active_id_to_slot.pop(owner, None)
            active_id_to_plate.pop(owner, None)
            active_id_to_suffix.pop(owner, None)
            captured_in.discard(owner)
            plate_enrichment.pop(owner, None)
            capture_reason.pop(owner, None)
            capture_open.pop(owner, None)
            best_score.pop(owner, None)
            best_text.pop(owner, None)
            best_det.pop(owner, None)
            ever_outside.pop(owner, None)
            last_seen_frame.pop(owner, None)
        return True
        ...
        while True:
            ...
            if do_heavy_this_frame:
                ...
                if not use_roi:
                    ...
                    gate_margin = GATE_CAPTURE_MARGIN_PX + 40

                    def should_run_ocr(box):
                        try:
                            x1, y1, x2, y2 = box
                        except Exception:
                            return True
                        ...
                        fx2 = x2 + d_rx
                        cx = (fx1 + fx2) / 2.0
                        return abs(cx - gate_x) <= gate_margin

                    ocr_filter = should_run_ocr

                all_lp_dets, all_lp_texts = lp_det.detect_frames([inf_roi], ocr_filter=ocr_filter)
            ...
            if tracked_detections.tracker_id is not None:
                for tid_val in tracked_detections.tracker_id:
                    try:
                        tid = int(tid_val)
            ...
            if do_heavy_this_frame:
                for idx, (det, text) in enumerate(zip(lp_dets_full, lp_texts)):
                    ...
                    if oid not in prev_inside:
                        ...
                        ever_outside[oid] = not is_in
                        if use_roi and is_in:
                            capture_open[oid] = capture_window_frames
                            capture_reason[oid] = "roi"
                            best_score[oid] = 0.0; best_text[oid] = ""; best_det[oid] = det
                        else:
                            capture_open.setdefault(oid, 0)
                        continue
                    ...
                    if is_in:
                        ...
                        inside_count[oid]  = 0
                        ever_outside[oid] = True

                    prev_was_inside = prev_inside.get(oid, False)
                    last_event_frame = last_event_f.get(oid, -10**9)
                    history_len = len(track_history.get(oid, []))
                    has_history = history_len >= MIN_HISTORY_FOR_CAPTURE
                    ...
                    if crossed_in_now and (frame_idx - last_event_frame >= event_gap_frames):
                        ...
                        capture_reason[oid] = "line_cross_in"
                        ever_outside[oid] = True
                    ...
                    if crossed_out_now and (frame_idx - last_event_frame >= event_gap_frames):
                        ...
                        plate_key_now = active_id_to_plate.get(oid) or plate_key(chosen_text)
                        suffix_lookup = suf_out or fields.get("suffix")
                        slot_to_free = lookup_slot_for_plate(oid, plate_key_now, suffix_lookup)

                        if finalize_exit_event(oid, slot_to_free, plate_key_now, suffix_lookup,
                                               chosen_text, region_class, suf_out, kana, city, engine_size, ts, det):
                    ...
                    if (not use_roi) and (not prev_inside[oid]) and is_in and has_history:
                        capture_open[oid] = capture_window_frames
                        capture_reason[oid] = "outside_to_inside"
                        ever_outside[oid] = True
                        best_score[oid] = 0.0; best_text[oid] = ""; best_det[oid] = det
                    ...
                    if (not use_roi) and (oid not in captured_in) and (capture_open.get(oid, 0) <= 0):
                        if (cx is not None and abs(cx - gate_x) <= GATE_CAPTURE_MARGIN_PX and
                                has_history and (ever_outside[oid] or crossed_in_now)):
                            capture_open[oid] = capture_window_frames
                            capture_reason[oid] = "near_gate"
                            best_score[oid] = 0.0; best_text[oid] = ""; best_det[oid] = det
                    ...
                    if is_in and (oid not in captured_in):
                        ...
                        reason_now = capture_reason.get(oid)
                        if capture_open.get(oid, 0) == 0 and reason_now and inside_count[oid] >= ENTER_STABLE_FRAMES:
                            if not use_roi:
                                if reason_now not in ("line_cross_in", "outside_to_inside", "near_gate"):
                                    capture_open[oid] = 1
                                    prev_inside[oid] = is_in
                                    continue
                                if reason_now == "near_gate" and not ever_outside[oid]:
                                    capture_open[oid] = 1
                                    prev_inside[oid] = is_in
                                    continue
                            chosen_text = best_text.get(oid,"") or text or ""
                    ...
                    if (not handled_exit_via_cross) and prev_inside[oid] and (not is_in) and outside_count[oid] >= EXIT_STABLE_FRAMES:
                        if (frame_idx - last_event_f.get(oid, -10**9) >= event_gap_frames):
                            fields = plate_enrichment.get(oid, {})
                            chosen_text = best_text.get(oid,"") or text or ""
                            if fields.get("raw_text"):
                                chosen_text = fields.get("raw_text")
                            ...
                            finalize_exit_event(oid, slot_to_free, plate_key_now, suffix_lookup,
                                                chosen_text, region_class, suf_out, kana, city, engine_size, ts, det)

                    prev_inside[oid] = is_in

            # Fallback: ensure OUT event fires if a tracked vehicle disappears without crossing
            if captured_in:
                for oid in list(captured_in):
                    if oid in active_tracker_ids:
                        continue
                    last_seen = last_seen_frame.get(oid)
                    if last_seen is None:
                        continue
                    if frame_idx - last_seen < MISSING_TRACK_EXIT_FRAMES:
                        continue
                    last_event_frame = last_event_f.get(oid, -10**9)
                    if frame_idx - last_event_frame < event_gap_frames:
                        continue
                    fields = plate_enrichment.get(oid, {})
                    chosen_text = best_text.get(oid, "") or fields.get("raw_text") or ""
                    region_class, suf_out, kana, city, engine_size = parse_plate_fields(chosen_text)
                    region_class = fields.get("region_class") or region_class
                    suf_out = fields.get("suffix") or suf_out
                    kana = fields.get("kana") or kana
