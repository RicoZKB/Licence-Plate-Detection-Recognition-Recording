# -*- coding: utf-8 -*-
# region14.py — ROI-cropped, one-shot capture with tolerant OCR parsing
...
# One-shot window + grace fill window (keeps OCRing a bit longer to fill columns)
CAPTURE_WINDOW_FRAMES    = 6
FILL_GRACE_FRAMES        = 12   # keep trying OCR after capture window until data is obtained

# While an object is idling inside *after* we’ve already logged it, throttle heavy OCR.
INSIDE_PROCESS_EVERY_N   = 8     # set to 1 to fully disable throttling

# One-shot accept rules
MIN_SHARPNESS_LOCK       = 60.0

# Gating (choose a good moment before opening capture window)
...
# Relaxed: accept city + class number + optional kana. Prefer left (JP) side if merged "JP | EN".
...
def main():
    ...
    # after-window grace filling
    grace_left = {}      # oid -> frames left to keep OCRing before writing IN
    pending_in = set()   # oids awaiting IN write (during grace)
    ...
    try:
        while True:
            ...
            # Draw overlays for current detections
            ...
            if do_heavy_this_frame:
                matched_oids = set()
                for det, text in zip(lp_dets_full, lp_texts):
                    # Association: prefer IoU with last bbox
                    ...
                    # skip writing if already locked (post-IN)
                    ...
                    if is_in and (oid not in captured_in):
                        ...
                        # within window, keep best suffix frame
                        ...
                        # when window closes, start grace-fill
                        if capture_open.get(oid, None) == 0 and inside_count[oid] >= ENTER_STABLE_FRAMES:
                            pending_in.add(oid)
                            grace_left[oid] = FILL_GRACE_FRAMES
                            # don’t write yet—let’s keep trying to fill columns
                            capture_open[oid] = -2  # mark as "pending write"

                    # ----- EXIT handling -----
                    if prev_inside[oid] and (not is_in) and outside_count[oid] >= EXIT_STABLE_FRAMES:
                        # If IN was still pending, force write before OUT
                        if oid in pending_in:
                            chosen_text = best_text.get(oid,"")
                            suf = plate_suffix(chosen_text) or last_suf.get(oid)
                            city = parse_city_kana(chosen_text)[0] or last_city.get(oid)
                            kana = parse_city_kana(chosen_text)[1] or last_kana.get(oid)
                            ...
                            print(f"[IN-FORCE] text='{chosen_text}' city='{city}' kana='{kana}' engine='{engine_size}'")
                            write_row_flush(csv_writer, csv_file,
                                            [ts, object_id, "car", "in", city or "", engine_size, kana or ""])
                            captured_in.add(oid)
                        ...
                        if frame_idx - last_event_f.get(oid, -10**9) >= MIN_EVENT_GAP_FRAMES:
                            ...
                            if slot_to_free is not None:
                                chosen_text = best_text.get(oid,"")  # last good we saw
                                ...
                                print(f"[OUT] text='{chosen_text}' city='{city}' kana='{kana}' engine='{engine_size_out}'")
                                write_row_flush(csv_writer, csv_file,
                                                [ts, object_id, "car", "out", city or "", engine_size_out, kana or ""])
            ...
            # ----- GRACE FILL: keep trying until we have fields, then write IN -----
            if do_heavy_this_frame and pending_in:
                to_write = []
                for oid in list(pending_in):
                    # try to improve best_text using last caches
                    chosen_text = best_text.get(oid, "")
                    suf = plate_suffix(chosen_text) or last_suf.get(oid)
                    city = parse_city_kana(chosen_text)[0] or last_city.get(oid)
                    kana = parse_city_kana(chosen_text)[1] or last_kana.get(oid)

                    # if we have at least a suffix (for engine size) OR we got city,
                    # go ahead and write the IN row
                    if suf or city or (grace_left.get(oid, 0) <= 0):
                        to_write.append((oid, chosen_text, suf, city, kana))
                    else:
                        # keep waiting a bit
                        grace_left[oid] = grace_left.get(oid, FILL_GRACE_FRAMES) - 1

                for oid, chosen_text, suf, city, kana in to_write:
                    # allocate slot
                    pk = plate_key(chosen_text) if suf else None
                    chosen_slot = None
                    ...
                    engine_size = engine_size_from_suffix(suf)
                    print(f"[IN] text='{chosen_text}' city='{city}' kana='{kana}' engine='{engine_size}'")
                    write_row_flush(csv_writer, csv_file,
                                    [ts, object_id, "car", "in", city or "", engine_size, kana or ""])
            ...
            # Display / write
            cv2.imshow("Parking (region14 ROI one-shot, gated+grace)", frame)
