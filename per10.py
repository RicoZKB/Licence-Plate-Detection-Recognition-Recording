# -*- coding: utf-8 -*-
# semi_per01.py — fast pipeline + YouTube option + terminal logging
# (uses kana-boosted LicencePlateDetection)
...
def _resolve_youtube_opencv_url(url: str, max_height: int = 480) -> str:
    try:
        ...
        print("[WARN] yt-dlp not installed. pip install yt-dlp")
        return None
...
def open_daily_csv():
    ...
    w = csv.writer(f)
    if new:
        w.writerow(["timestamp","object_id","vehicle_type","direction","region_class","suffix","kana","city"]); f.flush()
    return f, w, path
...
def log_terminal(event_ts, direction, slot_str, region_class, suffix, kana, city, raw_text):
    kd = kana or ""; cd = city or ""; rc = region_class or ""; sf = suffix or ""
    print(f"[{event_ts}] {direction.upper():3s}  slot={slot_str:>6s}  region_class='{rc}'  suffix='{sf}'  kana='{kd}'  city='{cd}'  raw=\"{raw_text}\"")
...
def draw_region(frame, rx, ry, rw, rh):
    ...
    cv2.putText(frame, "ROI (locked)" if LOCK_ROI else "ROI: WASD [ ] r",
...
def main():
    # source select
    chosen = [n for n,v in [("Camera",USE_CAMERA),("YouTube",USE_YOUTUBE),("File",USE_FILE)] if v]
    if len(chosen) != 1:
    ...
    if not cap.isOpened(): raise RuntimeError(f"Could not open source: {src}")

    using_camera = (src_name == "Camera")
    if using_camera:
    ...
    lp_det = LicencePlateDetection("models/best.pt", verbose=False, debug_ocr=PASS_OCR_DEBUG_TO_DETECT)
    ...
    try:
        while True:
            ...
            do_heavy = True
            if have_locked_inside and (frame_idx % INSIDE_PROCESS_EVERY_N != 0): do_heavy = False
            if frame_idx % DETECT_EVERY_N != 0: do_heavy = False
            ...
            if DRAW_BBOXES and do_heavy:
                draw_lp_boxes(frame, lp_dets_full, (0,255,0))
            if DRAW_BBOXES:
                if use_roi:
                ...
                for det, text in zip(lp_dets_full, lp_texts):
                    ...
                    if is_in and (oid not in captured_in):
                        if capture_open.get(oid, 0) > 0:
                            ...
                            if min_side < 18:
                                capture_open[oid] -= 1; prev_inside[oid] = is_in; continue
            ...
            cv2.imshow("Parking (semi_per01 + kana boost + YouTube)", frame)
