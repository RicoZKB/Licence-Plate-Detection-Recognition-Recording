...
GATE_CROSSING_THRESHOLD  = 10  # Minimum horizontal delta (px) to treat as a gate crossing
# =========================================================
...
def detect_gate_crossing(track_history_deque, gate_x, min_cross_px=10):
    """
    Detect if a track crossed the virtual gate line.
    Returns:
        crossed (bool): True when a sign change across the gate is observed.
        direction (str|None): 'left_to_right' or 'right_to_left'
    """
    if len(track_history_deque) < 2:
        return False, None

    points = list(track_history_deque)
    for i in range(len(points) - 1):
        ...
        crossed_line = (delta1 == 0 or delta2 == 0 or (delta1 < 0 < delta2) or (delta1 > 0 > delta2))
        moved_enough = abs(x2 - x1) >= min_cross_px or abs(delta1) <= min_cross_px or abs(delta2) <= min_cross_px

        if crossed_line and moved_enough:
            return True, 'left_to_right' if x2 > x1 else 'right_to_left'
...
def main():
    ...
    last_crossing_direction = {}  # oid -> 'left_to_right' or 'right_to_left'
    has_crossed_gate = {}  # oid -> True if already crossed (prevent duplicate events)
    ...
    try:
        while True:
            ...
            detections_sv = sv.Detections(xyxy=det_xyxy, confidence=det_conf)
            tracked_detections = byte_tracker.update_with_detections(detections_sv)

            if xyxy_list:
                # Update lp_dets_full with tracker IDs embedded
                for i, tid in enumerate(tracked_detections.tracker_id):
                    if i < len(lp_dets_full):
                        tid_val = int(tid) if tid is not None else -1
                        det_entry = lp_dets_full[i]
                        if isinstance(det_entry, (list, tuple, np.ndarray)):
                            base = list(det_entry[:4])
                        else:
                            xy = xyxy_from_det(det_entry)
                            base = list(xy) if xy else [0, 0, 0, 0]
                        lp_dets_full[i] = base + [tid_val]
            ...
            if do_heavy_this_frame:
                for det, text in zip(lp_dets_full, lp_texts):
                    ...
                    # Check if this track just crossed the gate line (trajectory-based)
                    crossed, crossing_direction = detect_gate_crossing(track_history[oid], gate_x, GATE_CROSSING_THRESHOLD)
                    ...
                    if oid not in prev_inside:
                        ...
                        has_crossed_gate[oid] = crossed
                        if crossed:
                            last_crossing_direction[oid] = crossing_direction
                        if is_in or crossed:
                    ...
                    # NEW: Detect gate crossing and handle bidirectional events
                    if crossed and crossing_direction:
                        # Only process if this is a NEW crossing (direction changed)
                        if last_crossing_direction.get(oid) != crossing_direction:
                            last_crossing_direction[oid] = crossing_direction
                            has_crossed_gate[oid] = True

                            # Determine if this crossing is IN or OUT based on gate orientation
                            # gate_right=True means right side is "inside":
                            #   - left_to_right = entering (IN)
                            #   - right_to_left = exiting (OUT)
                            # gate_right=False means left side is "inside":
                            #   - right_to_left = entering (IN)
                            #   - left_to_right = exiting (OUT)
                            if gate_right:
                                is_entry_crossing = (crossing_direction == 'left_to_right')
                            else:
                                is_entry_crossing = (crossing_direction == 'right_to_left')

                            # Debug: Print gate crossing detection
                            if PRINT_GATE_CROSSINGS:
                                was_captured = "YES" if oid in captured_in else "NO"
                                event_type = "IN" if is_entry_crossing else "OUT"
                                print(f"[GATE] tracker_id={oid} crossed {crossing_direction} → {event_type}, was_captured={was_captured}, cx={cx:.0f}, gate_right={gate_right}")

                            # Handle based on crossing type
                            if is_entry_crossing:
                                # ENTRY crossing: Open capture window for OCR
                                # If vehicle was already captured, this is a re-entry (log OUT first, then prepare for new IN)
                                if oid in captured_in:
                                    # Log implicit OUT (vehicle left and is now re-entering)
                                    ...
                                    if oid in captured_in:
                                        captured_in.remove(oid)

                                # Open capture window for (new) entry
                                capture_open[oid] = CAPTURE_WINDOW_FRAMES
                                best_score[oid] = 0.0
                                best_text[oid] = ""
                                best_det[oid] = det
                                ...
                                # EXIT crossing: Log OUT event if vehicle was captured
                                if oid in captured_in:
                                    pk_now = active_id_to_plate.get(oid)
                                    slot_to_free = None
                                    if pk_now and pk_now in active_plate_to_slot:
                                ...
                                # Don't open capture window on exit crossing
                                capture_open[oid] = 0

                    # Skip locked captures UNLESS we just crossed (which reopened the window)
                    if capture_open.get(oid, 0) == -1 and not crossed:
                        prev_inside[oid] = is_in
                        continue
                    ...
                    # Skip if we just handled exit via gate crossing
            ...
            if k == ord('o'):
                ...
                # Clear crossing direction history for all (force re-detection)
                last_crossing_direction.clear()
                has_crossed_gate.clear()
