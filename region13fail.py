...
CAPTURE_WINDOW_FRAMES    = 12

# While a car remains inside and already captured, only process heavy every N frames
INSIDE_PROCESS_EVERY_N   = 4

# One-shot accept rules
MIN_SHARPNESS_LOCK       = 45.0   # Laplacian variance to accept immediately

# ── One-shot gating: wait for a "perfect" moment ────────────────────────────
ENTER_DELAY_FRAMES       = 2
REQUIRE_FULLY_INSIDE     = False
FULLY_INSIDE_MARGIN_FRAC = 0.05
CENTER_BAND_FRAC         = 0.45
MIN_PLATE_AREA_FRAC      = 0.010
...
# Acceptance policy for committing a single IN row
ACCEPT_MIN_LEN           = 6    # letters/digits count to consider text usable
MAX_CAPTURE_WAIT_FRAMES  = 30   # additional frames to wait for better text
# =========================================================
...
CITY_KANA_RE = re.compile(
    u"^\\s*([^\\s\\d]+)\\s*([0-9]{3,4})\\s*([ぁ-ゖァ-ヿA-Za-z])",
    re.UNICODE,
)
...
def parse_city_kana_engine(text):
...
def plate_key(text):
    suf = plate_suffix(text or "");  city, _, _ = parse_city_kana_engine(text or "")
    if not suf: return None
    return f"{city if city else '?'}|{suf}"

def engine_size_from_triplet(engine_digits: str):
    return (engine_digits or "").strip() if engine_digits else ""
...
def open_daily_csv():
    ...
    if not new:
        try:
            # Use csv.reader to robustly check header and row width
            with open(path, "r", encoding="utf-8", errors="ignore", newline="") as rf:
                reader = csv.reader(rf)
                rows = []
                for i, row in enumerate(reader):
                    rows.append(row)
                    if i >= 1:  # header + first data row is enough
                        break
            header = rows[0] if rows else []
            first_data = rows[1] if len(rows) > 1 else None
            required = ["timestamp","object_id","plate_text","vehicle_type","direction","city","engine_size","kana"]

            rotate = False
            # Header mismatch against expected columns
            if header and AUTO_ROTATE_CSV_HEADER_MISMATCH:
                # if any required missing, or header length != expected
                if any(h not in header for h in required) or len(header) != len(required):
                    rotate = True
            # Row-width mismatch also forces rotation to keep schema consistent
            if (not rotate) and first_data is not None and AUTO_ROTATE_CSV_HEADER_MISMATCH:
            ...
            if rotate:
                ...
                os.replace(path, bak)
                print(f"[INFO] Rotated old CSV due to schema mismatch -> {bak}")
                new = True
...
def main():
    ...
    # Missed-detection accounting to emit OUT even if last frames had no boxes
    miss_count_in = {}
        ...
        while True:
            ...
            if do_heavy_this_frame:
                ...
                for det, text in zip(lp_dets_full, lp_texts):
                    ...
                    c_now, e_now, k_now = parse_city_kana_engine(text or "")
                    if c_now: last_city[oid] = c_now
                    if e_now: last_engine[oid] = e_now
                    ...
                    if is_in and (oid not in captured_in):
                        ...
                        if gate_ready.get(oid, False) and capture_open.get(oid, None) is None:
                            if gate_delay_left.get(oid, 0) > 0:
                                ...
                                capture_deadline_f[oid] = frame_idx + MAX_CAPTURE_WAIT_FRAMES
                        ...
                        if capture_open.get(oid, 0) and capture_open[oid] > 0:
                            ...
                            # Prefer texts that have a numeric suffix or are longer/more informative
                            cur_best = best_text.get(oid, "")
                            suffix_now = plate_suffix(text or "")
                            better = False
                            if suffix_now and score > best_score.get(oid, 0.0):
                                better = True
                            elif (not cur_best) and (text and len(text.strip()) >= 3):
                                better = True
                            elif text and (len(text.strip()) > len(cur_best.strip()) + 1):
                                better = True
                            if better:
                                best_score[oid] = max(score, best_score.get(oid, 0.0))
                                best_text[oid]  = text
                                best_det[oid]   = det

                            # Decide whether to stop window now
                            def _is_good_plate(txt: str) -> bool:
                                t = _normalize_text(txt or "")
                                if plate_suffix(t):
                                    return True
                                alnum = re.sub(r"[^0-9A-Za-z]", "", t)
                                return len(alnum) >= ACCEPT_MIN_LEN

                            if _is_good_plate(text) and (sharp >= MIN_SHARPNESS_LOCK or plate_suffix(text)):
                                capture_open[oid] = 0
                            else:
                                capture_open[oid] -= 1
                                # Extend until deadline to try to get better text
                                if capture_open[oid] <= 0 and frame_idx < capture_deadline_f.get(oid, -1):
                                    capture_open[oid] = 2

                        if capture_open.get(oid, 0) == 0 and inside_count[oid] >= ENTER_STABLE_FRAMES:
                            # build final fields with robust fallbacks
                            chosen_text = (best_text.get(oid,"") or text or "").strip()
                            if len(chosen_text) < 3 and last_suf.get(oid):
                                chosen_text = last_suf.get(oid)
                            suf = plate_suffix(chosen_text) or last_suf.get(oid)
                            city, engine_digits, kana = parse_city_kana_engine(chosen_text)
                            if not city: city = last_city.get(oid)
                            if not engine_digits: engine_digits = last_engine.get(oid)
                            ...
                            if chosen_slot is not None:
                                ...
                                engine_size = engine_size_from_triplet(engine_digits)
                                ...
                                capture_deadline_f.pop(oid, None)
                                if DRAW_BBOXES:
                    ...
                    if prev_inside[oid] and (not is_in) and outside_count[oid] >= EXIT_STABLE_FRAMES:
                        if frame_idx - last_event_f.get(oid, -10**9) >= MIN_EVENT_GAP_FRAMES:
                            ...
                            if slot_to_free is not None:
                                ...
                                engine_size_out = engine_size_from_triplet(engine_digits)
                ...
                # Handle unseen objects that were previously inside (timeout to OUT)
                try:
                    for oid in list(prev_inside.keys()):
                        if oid in matched_oids:
                            miss_count_in[oid] = 0
                            continue
                        # Only consider objects that were inside previously and not yet locked out
                        if prev_inside.get(oid, False):
                            miss_count_in[oid] = miss_count_in.get(oid, 0) + 1
                            if miss_count_in[oid] >= EXIT_STABLE_FRAMES:
                                # Synthesize an OUT event if sufficient gap since last event
                                if frame_idx - last_event_f.get(oid, -10**9) >= MIN_EVENT_GAP_FRAMES:
                                    pk_now = active_id_to_plate.get(oid)
                                    slot_to_free = None
                                    if pk_now and pk_now in active_plate_to_slot:
                                    ...
                                    if slot_to_free is None:
                                        slot_to_free = active_id_to_slot.pop(oid, None)
                                    if slot_to_free is not None:
                                        chosen_text = (best_text.get(oid, "") or "").strip()
                                        if len(chosen_text) < 3 and last_suf.get(oid):
                                            chosen_text = last_suf.get(oid)
                                        suf_out = plate_suffix(chosen_text) or last_suf.get(oid)
                                        city, engine_digits, kana = parse_city_kana_engine(chosen_text)
                                        ...
                                        slot_pool.release(slot_to_free)
                                        last_event_f[oid] = frame_idx
                                        # Visual hint near last known center
                                        lc = last_det.get(oid)
                                        if DRAW_BBOXES and lc is not None:
                                            cx = (lc[0] + lc[2]) / 2.0
                                            cy = (lc[1] + lc[3]) / 2.0
                                            cv2.putText(frame, f"OUT {object_id}", (int(cx), int(cy)),
                                                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,0,255), 2, cv2.LINE_AA)

                                    # Clear state for this oid
                                    for dct in (capture_open, best_score, best_text, best_det, gate_ready, gate_delay_left):
                                        dct.pop(oid, None)
                                    if oid in captured_in: captured_in.remove(oid)
                                    last_det.pop(oid, None)
                                    capture_deadline_f.pop(oid, None)
                                    prev_inside[oid] = False
                                    miss_count_in[oid] = 0
                except Exception:
                    pass
