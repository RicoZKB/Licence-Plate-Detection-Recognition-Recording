# -*- coding: utf-8 -*-
# per7_format.py — per5_format + ByteTrack for robust tracking across frames
# - All working ROI/OCR/CSV logic from per5_format.py
# - ByteTrack replaces stable_key for motion-based tracking
# - Unique color visualization per tracker ID
# - Enhanced track history trails
...
GATE_CROSSING_THRESHOLD  = 10  # Pixels tolerance for gate crossing detection
# =========================================================
...
def detect_gate_crossing(track_history_deque, gate_x, threshold=10):
    """
    Detect if trajectory crossed the gate line and determine direction.
    Returns: (crossed, direction)
      crossed: True if crossed the gate
      direction: 'left_to_right' or 'right_to_left' or None
    """
    if len(track_history_deque) < 2:
        return False, None

    points = list(track_history_deque)
    # Check if the trajectory crosses the gate line
    for i in range(len(points) - 1):
        x1, _ = points[i]
        x2, _ = points[i + 1]

        # Check if crossed the gate (within threshold)
        if abs(x1 - gate_x) <= threshold or abs(x2 - gate_x) <= threshold:
    ...
    try:
        while True:
            ...
            if do_heavy_this_frame:
                ...
                # Convert to supervision format and update ByteTrack
                if len(lp_dets_full) > 0:
                    xyxy_list = []
                    ...
                    if xyxy_list:
                        detections = sv.Detections(
                            ...
                            confidence=np.ones(len(xyxy_list), dtype=np.float32) * 0.9
                        )
                        detections = byte_tracker.update_with_detections(detections)
                        # Update lp_dets_full with tracker IDs embedded
                        for i, tid in enumerate(detections.tracker_id):
                            if i < len(lp_dets_full) and isinstance(lp_dets_full[i], list):
                                lp_dets_full[i] = lp_dets_full[i] + [int(tid) if tid is not None else -1]
            else:
                lp_dets_full, lp_texts = [], []
