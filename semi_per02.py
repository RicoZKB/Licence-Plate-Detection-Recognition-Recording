# -*- coding: utf-8 -*-
# semi_per02.py — ROI/Gate one-shot capture with kana-friendly OCR voting,
# terminal event logs, YouTube option, and robust on-frame drawing (no pillow).
#
# NOTE:
# - We DO NOT call detector.draw_bboxes() anymore. We draw boxes/text here,
#   after normalizing bbox formats, so OpenCV never crashes on weird types.
# - Kana will always show correctly in the TERMINAL logs; OpenCV overlay text
#   may not render kana on some systems (depends on your setup). If you want
#   on-frame kana later, we can add Pillow with a JP font, but this version
#   focuses on robustness + terminal visibility.

import os, cv2, csv, re, time
from datetime import datetime

# If your class is in detections.py, switch to:
...
PRINT_RAW_OCR_FOUND      = False          # noisy; prints raw OCR hits
PASS_OCR_DEBUG_TO_DETECT = False          # passes debug_ocr=True into detector
...
def xyxy_from_det(det):
    """Return (x1,y1,x2,y2) as floats from various det formats."""
...
def draw_lp_boxes_safe(frame, dets, texts):
    """Robust drawing: normalize any bbox format to ints before cv2 drawing."""
    if not DRAW_BBOXES: 
        return frame
    if dets is None: 
        return frame
    for det, text in zip(dets, texts or []):
        xy = xyxy_from_det(det)
        if not xy:
            continue
        x1,y1,x2,y2 = xy
        ...
        # OpenCV may not render kana, but we still put the raw string (digits/hyphen show fine)
        label = str(text or "")
        cv2.putText(frame, label, (x1, max(10, y1 - 8)),
...
def main():
    ...
    try:
        while True:
                    ...
                    if is_in and (oid not in captured_in):
                                ...
                                if min_side < 18:
                                    ...
                                    if sharp < 25.0:
                                        ...
                                        if norm:
                                            ...
                                            bonus = 0
                                            if KANA_RE.search(norm): bonus += 1
                                            if plate_suffix(norm):   bonus += 2
                        ...
                        if capture_open.get(oid, 0) == 0 and inside_count[oid] >= ENTER_STABLE_FRAMES:
                            ...
                            if chosen_slot is not None:
                                ...
                                log_terminal(ts, "in", object_id, region_class, suf, kana, city, chosen_text)
                    ...
                    if prev_inside[oid] and (not is_in) and outside_count[oid] >= EXIT_STABLE_FRAMES:
                        if frame_idx - last_event_f.get(oid, -10**9) >= MIN_EVENT_GAP_FRAMES:
                            ...
                            if slot_to_free is not None:
                                ...
                                log_terminal(ts, "out", object_id, region_class, suf_out, kana, city, chosen_text)
            ...
            cv2.imshow("Parking (semi_per02 robust draw)", frame)
