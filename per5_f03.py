...
# - Gate-based virtual line with centroid tracking (IN/OUT events)
# - One-shot OCR on ENTER (pick best sharp/sharpness+valid suffix), then throttle while inside
# - Immediate CSV flush on every write (daily file)
...
PRINT_RAW_OCR_FOUND      = False  # print per-detection OCR strings when kana/suffix present
...
USE_ROI                  = False
ENTRY_LINE_X_RATIO       = 0.50   # <<< centered gate (matches your screenshot)
...
class SimpleTracker:
    """Greedy nearest-neighbor tracker that keeps one persistent oid per plate."""
    def __init__(self, start_id=100, max_distance=70.0, max_missed=12):
        self.next_id = int(start_id)
        self.max_distance = float(max_distance)
        self.max_missed = int(max_missed)
        self.tracks = {}  # oid -> {'xy':(x,y), 'missed':int}
    ...
    def update(self, detections, frame_idx, gate_x):
        ...
        for det in detections:
            cx, cy = det.get('cx', 0.0), det.get('cy', 0.0)
            best_oid, best_dist = None, float('inf')
            for oid, tr in self.tracks.items():
                if tr['missed'] > self.max_missed or oid in used:
                    continue
                tx, ty = tr['xy']
            ...
            if best_oid is None:
                ...
                self.tracks[best_oid] = {
                    'xy': (cx, cy),
                    'missed': 0,
                    'last_frame': frame_idx,
                    'side': 'R' if cx >= gate_x else 'L',
                    'side_streak': 1,
                }
            else:
                ...
                if side_now == tr.get('side'):
                    tr['side_streak'] = tr.get('side_streak', 1) + 1
                else:
                    tr['side'] = side_now
                    tr['side_streak'] = 1
...
def main():
    ...
    tracker = SimpleTracker(start_id=START_ID_AT, max_distance=80.0, max_missed=15)
    prev_inside = {}
    ...
    try:
        while True:
            ...
            have_locked_inside = bool(active_plate_to_slot)
            ...
            if DRAW_BBOXES:
                if use_roi:
                    ...
                    label_x = gx + 12 if gate_right else gx - 120
                    label_x = max(10, min(label_x, W-110))
                    cv2.putText(frame, f"in: {total_in}", (int(label_x), 48),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 200, 0), 2, cv2.LINE_AA)
                    cv2.putText(frame, f"out: {total_out}", (int(label_x), 78),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 0, 255), 2, cv2.LINE_AA)

            ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            gate_x = int(ENTRY_LINE_X_RATIO * W)

            # Process detections -> track -> events
            det_items = []
            if do_heavy_this_frame and lp_dets_full:
                for idx, det in enumerate(lp_dets_full):
                    text = lp_texts[idx] if idx < len(lp_texts) else ""
                    cx, cy = center_from_det(det)
                    if cx is None or cy is None:
                        continue
                    det_items.append({'det': det, 'text': text, 'cx': cx, 'cy': cy})

            tracked_items = tracker.update(det_items, frame_idx, gate_x)

            # prune bookkeeping for tracks that vanished
            active_ids = set(tracker.tracks.keys())
            stale_oids = [oid for oid in list(prev_inside.keys()) if oid not in active_ids]
            for oid in stale_oids:
                prev_inside.pop(oid, None)
                inside_count.pop(oid, None)
                outside_count.pop(oid, None)
                last_event_f.pop(oid, None)
                capture_open.pop(oid, None)
                best_score.pop(oid, None)
                best_text.pop(oid, None)
                best_det.pop(oid, None)
                captured_in.discard(oid)
                active_id_to_plate.pop(oid, None)
                active_id_to_slot.pop(oid, None)
            ...
            if tracked_items:
                for item in tracked_items:
                    ...
                    dir_in = '->' if gate_right else '<-'
                    dir_out = '<-' if gate_right else '->'
                    ...
                    if is_in and (oid not in captured_in):
                        ...
                        if capture_open.get(oid, 0) == 0 and inside_count[oid] >= ENTER_STABLE_FRAMES:
                            ...
                            # plate already owns a slot (duplicate attach)
                            if pk and pk in active_plate_to_slot:
                                ...
                                if DRAW_BBOXES:
                                    cv2.putText(frame, f"IN {chosen_slot:02d} {dir_in}", (int(cx), int(cy)),
                            ...
                            # duplicate attach via suffix only
                            chosen_slot = None
                            ...
                            if chosen_slot is not None:
                                ...
                                total_in += 1
                                if DRAW_BBOXES:
                                    cv2.putText(frame, f"IN {object_id} {dir_in}", (int(cx), int(cy)),
                                                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,0), 2, cv2.LINE_AA)
                                log_terminal(ts, "in", f"{object_id}({suf})" if suf else object_id,
                                             region_class, suf, kana, city, chosen_text)

                    # EXIT handling
                    if prev_inside.get(oid, False) and (not is_in) and outside_count.get(oid, 0) >= EXIT_STABLE_FRAMES:
                        if frame_idx - last_event_f.get(oid, -10**9) >= MIN_EVENT_GAP_FRAMES:
                            ...
                            if slot_to_free is not None:
                                ...
                                total_out += 1
                                if DRAW_BBOXES:
                                    cv2.putText(frame, f"OUT {object_id} {dir_out}", (int(cx), int(cy)),
            ...
            # Display / write
            cv2.imshow("Parking (per5_format — gate + one-shot)", frame)
