# -*- coding: utf-8 -*-
# region06.py — Pre-band scanning + one-shot capture + throttled inside OCR
# - Pre-gate scan: look only in a shallow band above the ROI every N frames (cropped+downscaled)
# - When a plate is detected in that band, open a short capture window and do the heavy pass
# - Log once on ENTER (lease slot) and once on EXIT (free slot)
# - Immediate CSV flush
...
# ROI as ratios of frame (x, y, w, h). Move with WASD, resize with [ ]
REGION_XYWH_RATIO     = (0.30, 0.62, 0.40, 0.22)   # made a bit smaller by default
...
# One-shot capture window after first entry frame
CAPTURE_WINDOW_FRAMES = 8

# Throttle while inside (process every N frames)
INSIDE_PROCESS_EVERY_N = 4

# ---------- NEW: pre-gate band scan ----------
# Scan band every N frames (light), only area just above ROI
PREGATE_EVERY_N       = 3
PREGATE_BAND_TOP      = 0.22      # band starts this fraction of H above ROI.y (clamped at 0)
PREGATE_EXTRA_W       = 0.10      # widen the band horizontally by this fraction of W on both sides
PREGATE_TARGET_W      = 448       # downscaled width for the pre-gate crop (speed!)

# Plate suffix pattern (reduce false positives)
...
def draw_region(frame, rx, ry, rw, rh):
    ...
    cv2.putText(frame, "ROI: WASD move, [ ] resize, r reset",
                (rx+6, max(18, ry-6)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0,255,120), 2, cv2.LINE_AA)
...
def main():
    ...
    # One-shot capture
    capture_open = {}      # oid -> frames left
    best_score  = {}
    best_text   = {}
    best_det    = {}
    captured_in = set()    # oid that has already logged IN

    # Pre-gate arming (we only open capture windows after a pre-band hit)
    pregate_armed_until = 0  # frame index threshold; > frame_idx means "do heavy now"
    ...
    try:
        while True:
            ...
            if not ok:
                break
            # always pull the latest frame
            ok, frame = cap.retrieve()
            if not ok:
                break
            frame_idx += 1; fcount += 1

            # Compute pre-gate band (pixels)
            band_top    = max(0, int(ry - PREGATE_BAND_TOP*H))
            band_bottom = ry + rh
            band_left   = max(0, int(rx - PREGATE_EXTRA_W*W))
            band_right  = min(W, int(rx + rw + PREGATE_EXTRA_W*W))
            band_w      = max(10, band_right - band_left)
            band_h      = max(10, band_bottom - band_top)

            # Decide whether to run heavy pass this frame
            have_inside_captured = any(oid in captured_in and prev_inside.get(oid, False) for oid in prev_inside.keys())
            do_heavy = False

            # If we're inside and throttling, only heavy every INSIDE_PROCESS_EVERY_N
            if have_inside_captured:
                do_heavy = (frame_idx % INSIDE_PROCESS_EVERY_N == 0)
            # If pre-gate is armed, do heavy for a few frames to catch the best
            if frame_idx <= pregate_armed_until:
                do_heavy = True

            # If not armed and not inside-throttle frame, do a cheap pre-band probe every N frames
            if not do_heavy and (frame_idx % PREGATE_EVERY_N == 0):
                band = frame[band_top:band_bottom, band_left:band_right]
                if band.size > 0:
                    # downscale band for speed and remember scale/offset
                    scale = PREGATE_TARGET_W / float(band.shape[1])
                    if scale < 1.0:
                        band_small = cv2.resize(band, (int(band.shape[1]*scale), int(band.shape[0]*scale)), interpolation=cv2.INTER_AREA)
                    else:
                        band_small = band
                        scale = 1.0
                    all_bd, all_bt = lp_det.detect_frames([band_small])
                    bdets, btexts = all_bd[0], all_bt[0]
                    # If we see any plausible plate suffix, arm heavy processing for a short burst
                    has_candidate = False
                    for d, t in zip(bdets, btexts):
                        if plate_suffix(t or ""):
                            has_candidate = True
                            break
                    if has_candidate:
                        pregate_armed_until = frame_idx + CAPTURE_WINDOW_FRAMES  # heavy for next few frames
            ...
            # Optional car boxes
            car_dets = []
            if USE_CAR_DETECTOR and do_heavy:
            ...
            if DRAW_BBOXES:
                ...
                # show pre-gate band (thin)
                cv2.rectangle(frame, (band_left, band_top), (band_right, band_bottom), (40, 180, 255), 1)

            ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

            # Process detections (only when heavy ran)
            if do_heavy:
                for det, text in zip(lp_dets, lp_texts):
                    ...
                    if oid not in prev_inside:
                        ...
                        # If the very first time we see it and it's near ROI, open capture window
                        if is_in:
                    ...
                    # capture window while inside and not yet captured
            ...
            if SHOW_FPS:
                ...
                tag = " (heavy)" if do_heavy else " (preband/throttle)"
                cv2.putText(frame, f"FPS ~ {fps_est:.1f}{tag}", (10, 24),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2, cv2.LINE_AA)

            # Display / write
            cv2.imshow("Parking (region06 pre-band)", frame)
