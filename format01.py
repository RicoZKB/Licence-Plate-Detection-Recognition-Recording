...
# - EXIT now requires OCR confirmation on the exit side before logging OUT
...
EXIT_CONFIRM_WINDOW      = 8      # frames to wait for exit-side OCR confirmation
...
MIN_EXIT_SHARPNESS       = 50.0   # Slightly lower threshold for exit confirmation
...
CITY_KANA_RE = re.compile(u"^\\s*([^\\s\\d]+)\\s*[0-9]{2,4}.*?([あ-ゖぁ-ヿA-Za-z])", re.UNICODE)
...
REGION_WITH_NUM_RE = re.compile(u"([あ-ゖぁ-ヿ一-龯A-Za-z]+)\\s*([0-9]{2,4})", re.UNICODE)
def parse_plate_fields(text):
    ...
    kana = None; kana_matches = re.findall(u"[あ-ゖぁ-ヿ]", t)
...
def main():
    ...
    # one-shot bookkeeping for ENTRY
    ...
    # EXIT confirmation bookkeeping
    exit_pending = {}           # oid -> True when crossed but waiting for OCR confirm
    exit_best_text = {}         # oid -> best exit-side OCR
    exit_best_score = {}        # oid -> score for exit OCR
    exit_confirm_frames = {}    # oid -> frames to wait for exit OCR
    exit_best_det = {}          # oid -> best detection on exit side
    ...
    try:
        while True:
            ...
            if do_heavy_this_frame:
                for det, text in zip(lp_dets_full, lp_texts):
                    ...
                    if PRINT_RAW_OCR_FOUND:
                        ...
                        if norm and (re.search(u"[あ-ゖぁ-ヿ]", norm) or plate_suffix(norm)):
                    ...
                    # skip anything already locked (-1) for entry
                    if capture_open.get(oid, 0) == -1:
                        # But still collect exit OCR if in exit pending state
                        if exit_pending.get(oid) and (not is_in) and exit_confirm_frames.get(oid, 0) > 0:
                            ...
                            if min_side >= 18:
                                ...
                                if sharp >= 25.0:
                                    ...
                                    if score > exit_best_score.get(oid, 0.0) and plate_suffix(text or ""):
                                        exit_best_score[oid] = score
                                        exit_best_text[oid] = text
                                        exit_best_det[oid] = det
                            
                            exit_confirm_frames[oid] -= 1
                    ...
                    # one-shot capture maintenance for ENTRY
                    ...
                    # ========== EXIT handling with OCR confirmation ==========
                    if prev_inside[oid] and (not is_in) and outside_count[oid] >= EXIT_STABLE_FRAMES:
                        if frame_idx - last_event_f.get(oid, -10**9) >= MIN_EVENT_GAP_FRAMES:
                            ...
                            # Mark as pending exit if not already
                            if oid not in exit_pending:
                                exit_pending[oid] = True
                                exit_best_text[oid] = ""
                                exit_best_score[oid] = 0.0
                                exit_confirm_frames[oid] = EXIT_CONFIRM_WINDOW
                                exit_best_det[oid] = det
                            
                            # Collect OCR on exit side
                            if exit_pending.get(oid) and exit_confirm_frames.get(oid, 0) > 0:
                                x1, y1, x2, y2 = map(int, xyxy_from_det(det))
                                plate_img = frame[max(0,y1):min(H,y2), max(0,x1):min(W,x2)]
                            ...
                            # Confirm exit when we have valid OCR OR window expired with something
                            if exit_pending.get(oid):
                                confirmed = False
                                exit_text = exit_best_text.get(oid, "")
                                
                                # Immediate confirm if sharp + valid suffix
                                if exit_text and plate_suffix(exit_text):
                                    exit_det = exit_best_det.get(oid, det)
                                    x1, y1, x2, y2 = map(int, xyxy_from_det(exit_det))
                                    plate_img = frame[max(0,y1):min(H,y2), max(0,x1):min(W,x2)]
                                    sharp = variance_of_laplacian(plate_img)
                                    if sharp >= MIN_EXIT_SHARPNESS:
                                        confirmed = True
                                
                                # Or window expired with any valid suffix
                                if exit_confirm_frames.get(oid, 0) <= 0 and exit_text and plate_suffix(exit_text):
                                    confirmed = True
                                
                                if confirmed:
                                    # Now process the actual OUT event
                                    ...
                                    if slot_to_free is not None:
                                        region_class, suf_out, kana, city, engine_size = parse_plate_fields(exit_text)
                                        # CSV write (new format). object_id is slot only.
                                        ...
                                        log_terminal(ts, "out", f"{object_id}({suf_out})" if suf_out else object_id,
                                                     region_class, suf_out, kana, city, exit_text)

                                        if suf_out and active_suffix_to_slot.get(suf_out) == slot_to_free:
                                            active_suffix_to_slot.pop(suf_out, None)

                                    # Clear all exit tracking
                                    exit_pending.pop(oid, None)
                                    exit_best_text.pop(oid, None)
                                    exit_best_score.pop(oid, None)
                                    exit_confirm_frames.pop(oid, None)
                                    exit_best_det.pop(oid, None)
            ...
            # Display / write
            cv2.imshow("Parking (semi_per01 ROI one-shot + Exit OCR Confirm)", frame)
