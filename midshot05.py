# -*- coding: utf-8 -*-
# region07.py — ROI-cropped, one-shot plate capture; STRICT skip after lock
# - Runs detector/OCR only INSIDE the yellow ROI
# - Picks a single "great clear moment" (centered + sharp + valid suffix)
# - After logging once, DOES NOT run detector again until EXIT is confirmed
...
# ROI control (WASD/[ ]/r enabled when False)
LOCK_ROI                 = False
...
# One-shot window (frames we keep looking once we first see a candidate)
CAPTURE_WINDOW_FRAMES    = 6

# After we captured once, skip ALL heavy work until the object exits
STRICT_SKIP_AFTER_LOCK   = True

# "Great clear moment" filters
PLATE_SUFFIX_RE          = re.compile(r"([0-9]{2,3}-[0-9]{2})")
MIN_SHARPNESS_LOCK       = 60.0         # Laplacian variance
MID_BAND_RATIO_Y         = 0.50         # mid line (0..1 of ROI height)
MID_BAND_TOL             = 0.20         # must be within ±20% of ROI height around mid
MIN_PLATE_AREA_RATIO     = 0.012        # bbox area / ROI area required to consider
...
# ---------- helpers for "great clear moment" ----------
def passes_moment_filters(det, text, rx, ry, rw, rh, frame, H, W):
    """Center near mid, big enough area, valid suffix, and sharp."""
    xy = xyxy_from_det(det)
    if not xy: return False, 0.0
    x1,y1,x2,y2 = map(int, xy)
    cx, cy = (x1+x2)/2.0, (y1+y2)/2.0

    # must be inside ROI
    if not (rx <= cx <= rx+rw and ry <= cy <= ry+rh): return False, 0.0

    # near the mid band
    band_y = ry + rh*MID_BAND_RATIO_Y
    if abs(cy - band_y) > (rh*MID_BAND_TOL): return False, 0.0

    # area filter
    roi_area = float(rw*rh)
    if roi_area <= 0: return False, 0.0
    if (bbox_area(det) / roi_area) < MIN_PLATE_AREA_RATIO: return False, 0.0

    # text must contain numeric suffix like 20-56
    if not plate_suffix(text or ""): return False, 0.0

    # sharpness score×area as quality
    ...
    # final sharpness gate
    if sharp < MIN_SHARPNESS_LOCK: return False, score
    return True, score
...
def main():
    ...
    # global lock to skip heavy completely while something is inside & logged
    skip_heavy_until_exit = False
    ...
    try:
        while True:
            ...
            # Decide if we run detector this frame
            do_heavy_this_frame = not skip_heavy_until_exit
            ...
            if USE_CAR_DETECTOR and do_heavy_this_frame:
                ...
                for d in car_dets: car_dets_full.append(offset_det(d, rx, ry))

            # Draw overlays
            if DRAW_BBOXES and do_heavy_this_frame:
                draw_lp_boxes(frame, lp_dets_full, color=(0,255,0))
                if USE_CAR_DETECTOR: draw_lp_boxes(frame, car_dets_full, color=(255,200,0))
            if DRAW_BBOXES:
                draw_region(frame, rx, ry, rw, rh)

            ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

            # Process detections (only when heavy actually ran)
            if do_heavy_this_frame:
                for det, text in zip(lp_dets_full, lp_texts):
                    ...
                    # already fully locked? (skip updates)
                    if capture_open.get(oid, 0) == -1:
                        prev_inside[oid] = is_in
                        continue

                    # one-shot capture maintenance — only accept the GREAT moment
                    if is_in and (oid not in captured_in):
                        if capture_open.get(oid, 0) > 0:
                            passed, score = passes_moment_filters(det, text, rx, ry, rw, rh, frame, H, W)
                            if passed and score > best_score.get(oid, 0.0):
                                best_score[oid] = score
                                best_text[oid]  = text
                                best_det[oid]   = det
                                # as soon as a GREAT moment happens, close immediately
                                capture_open[oid] = 1
                            else:
                                capture_open[oid] -= 1

                        # window closes -> log IN exactly once
                        if capture_open.get(oid, 0) == 0 and inside_count[oid] >= ENTER_STABLE_FRAMES:
                            ...
                            if chosen_slot is not None:
                                ...
                                capture_open[oid] = -1  # LOCKED for this oid

                                # *** STRICT GLOBAL SKIP until exit ***
                                if STRICT_SKIP_AFTER_LOCK:
                                    skip_heavy_until_exit = True
                    ...
                    if prev_inside[oid] and (not is_in) and outside_count[oid] >= EXIT_STABLE_FRAMES:
                        if frame_idx - last_event_f.get(oid, -10**9) >= MIN_EVENT_GAP_FRAMES:
                            ...
                            # allow heavy again after full EXIT
                            skip_heavy_until_exit = False
            ...
            if SHOW_FPS:
                ...
                tag = "" if (not skip_heavy_until_exit) else "  (locked: skip)"
                cv2.putText(frame, f"FPS ~ {fps_est:.1f}{tag}", (10, 24),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2, cv2.LINE_AA)

            # Display / write
            cv2.imshow("Parking (region07 ROI one-shot)", frame)
            if out is not None and WRITE_VIDEO: out.write(frame)

            # ROI keyboard control if not locked
