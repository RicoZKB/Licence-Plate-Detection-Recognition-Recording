# -*- coding: utf-8 -*-
# region07.py — ROI-cropped, one-shot plate capture + throttled OCR while inside (fast tuned)
...
# ROI lock (set False to resize with keys)
LOCK_ROI                 = False   # was True
...
# ── New: Outside throttling ────────────────────────────────────────────────
# When no object is confirmed inside yet, only scan every N frames
SCAN_OUTSIDE_EVERY_N     = 5   # 5→ fewer scans when empty = faster

# ── New: Motion gate inside ROI (cheap) ───────────────────────────────────
USE_MOTION_GATE          = True
MOTION_BLUR_SIZE         = 5
MOTION_ABS_THRESH        = 14
MOTION_RATIO_MIN         = 0.006  # require at least this changed fraction to run detector this frame

# ── New: draw throttling ──────────────────────────────────────────────────
DRAW_EVERY_N             = 3   # draw overlays every N frames (0/1 = draw every frame)
...
def open_daily_csv():
    ...
    w = csv.writer(f)
    if new:
        w.writerow(["timestamp","object_id","vehicle_type","direction","city","kana"]); f.flush()
    return f, w, path
...
def center_from_det(det):
    ...
    x1,y1,x2,y2 = xy;  return (x1+x2)/2.0, (y1+y2)/2.0

def bbox_area(det):
    xy = xyxy_from_det(det)
    if not xy: return 0.0
    x1,y1,x2,y2 = xy;  return max(0, x2-x1) * max(0, y2-y1)
...
def draw_region(frame, rx, ry, rw, rh):
    ...
    cv2.putText(frame, msg, (rx+6, max(18, ry-6)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255), 1, cv2.LINE_AA)
...
def main():
    ...
    try:
        while True:
            ...
            # ── Determine if anyone is confirmed "inside" already ──
            someone_inside = any(prev_inside.values()) if prev_inside else False
            have_locked_inside = any((oid in captured_in) and prev_inside.get(oid, False) for oid in prev_inside.keys())

            # ── Cheap motion gate in ROI (skip heavy work if static) ──
            motion_ok = True
            if USE_MOTION_GATE:
                ...
                if prev_gray is not None and prev_gray.shape == gray.shape:
                    ...
                    motion_ok = (motion_ratio >= MOTION_RATIO_MIN)
                prev_gray = gray

            # ── Decide whether to run heavy detection this frame ──
            do_heavy_this_frame = True

            # Outside throttling when no inside yet
            if not someone_inside and (frame_idx % SCAN_OUTSIDE_EVERY_N != 0):
                do_heavy_this_frame = False

            # Throttle while car remains inside and we already captured
            if have_locked_inside and (frame_idx % INSIDE_PROCESS_EVERY_N != 0):
                do_heavy_this_frame = False

            # Motion gate
            if USE_MOTION_GATE and not motion_ok:
                do_heavy_this_frame = False
            ...
            # Draw overlays (throttled)
            draw_now = (DRAW_EVERY_N in (0,1)) or (frame_idx % DRAW_EVERY_N == 0)
            if DRAW_BBOXES and draw_now and do_heavy_this_frame:
                draw_lp_boxes(frame, lp_dets_full, color=(0,255,0))
                if USE_CAR_DETECTOR:
                    draw_lp_boxes(frame, car_dets_full, color=(255,200,0))
            if DRAW_BBOXES and draw_now:
                ...
                for det, text in zip(lp_dets_full, lp_texts):
                    ...
                    if is_in and (oid not in captured_in):
                        if capture_open.get(oid, 0) > 0:
                            ...
                            plate_img = frame[max(0,y1):min(int(H),y2), max(0,x1):min(int(W),x2)]
            ...
            if SHOW_FPS:
                ...
                tag = "" if do_heavy_this_frame else "  (skip)"
                cv2.putText(frame, f"FPS ~ {fps_est:.1f}{tag}", (10, 24),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2, cv2.LINE_AA)

            # Display / write
            cv2.imshow("Parking (region07 ROI one-shot, fast)", frame)
