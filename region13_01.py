# -*- coding: utf-8 -*-
# region11.py — ROI-cropped, one-shot plate capture with "perfect-moment" gate
# Logs plate data robustly using tolerant parsing + last-seen fallback
...
def open_daily_csv():
    ...
    w = csv.writer(f)
    if new:
        # keep your original header (no engine_size column)
...
def main():
    ...
    try:
        while True:
            ...
            # Display / write
            cv2.imshow("Parking (region11 ROI one-shot, gated)", frame)
