...
INSIDE_PROCESS_EVERY_N_CAMERA = 1
        ...
        while True:
            ...
            if do_heavy_this_frame:
                for idx, (det, text) in enumerate(zip(lp_dets_full, lp_texts)):
                    ...
                    if crossed_in_now and (not prev_was_inside) and (frame_idx - last_event_frame >= MIN_EVENT_GAP_FRAMES):
                        ...
                        if oid in captured_in:
                            ...
                            if slot_to_free is None:
                                ...
                                allowed_out = suffix_event_allowed(suffix_event_history, suf_out, "out", frame_idx, MIN_EVENT_GAP_FRAMES)
                                if allowed_out:
                    ...
                    if crossed_out_now and prev_was_inside and (frame_idx - last_event_frame >= MIN_EVENT_GAP_FRAMES):
                        if PRINT_GATE_CROSSINGS:
                    ...
                    if is_in and (oid not in captured_in):
                        ...
                        if capture_open.get(oid, 0) == 0 and inside_count[oid] >= ENTER_STABLE_FRAMES:
                            ...
                            if chosen_slot is not None:
                                ...
                                allowed_in = suffix_event_allowed(suffix_event_history, suf, "in", frame_idx, MIN_EVENT_GAP_FRAMES)
                                if allowed_in:
                                    write_row_flush(csv_writer, csv_file,
                ...
                for oid in oids_to_exit:
                    ...
                    if slot_to_free is not None:
                        ...
                        if fields.get("raw_text"):
                            chosen_text = fields.get("raw_text")
                        object_id = f"{slot_to_free:02d}"
                        if suffix_event_allowed(suffix_event_history, suf_out, "out", frame_idx, MIN_EVENT_GAP_FRAMES):
                            write_row_flush(csv_writer, csv_file,
                                            [ts, object_id, "car", "out", city or "", engine_size or "", kana or "", suf_out or ""])
                        last_event_f[oid] = frame_idx
                        slot_pool.release(slot_to_free)
