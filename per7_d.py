                    ...
                    if crossed and crossing_direction:
                        ...
                        if last_crossing_direction.get(oid) != crossing_direction:
                            ...
                            if PRINT_GATE_CROSSINGS:
                                ...
                                print(f"[GATE] tracker_id={oid} crossed {crossing_direction}, was_captured={was_captured}, cx={cx:.0f}")

                            # Handle EXIT event when vehicle was previously captured and crosses in ANY new direction
                            # This allows the same vehicle to exit and re-enter from either direction
                            if oid in captured_in:
                                ...
                                # Clear state for fresh entry
                                active_id_to_plate.pop(oid, None)
                                active_id_to_suffix.pop(oid, None)
                                if oid in captured_in:
                                    captured_in.remove(oid)

                            # UNLOCK the capture window for fresh OCR (both entry and re-entry after exit)
                            capture_open[oid] = CAPTURE_WINDOW_FRAMES
                            best_score[oid] = 0.0
                            best_text[oid] = ""
                            best_det[oid] = det
