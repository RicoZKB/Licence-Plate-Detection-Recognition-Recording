# -*- coding: utf-8 -*-
# tinout_event.py — Optimized single-event processing with enhanced tracking
#
# KEY OPTIMIZATIONS (FPS improvements from per6_format.py):
# 1. EVENT MODE: Configurable single-direction tracking (in/out/both)
#    - Set EVENT_MODE to "in", "out", or "both" for focused processing
#    - Reduces overhead by skipping unwanted event processing
#
# 2. OCR PERFORMANCE:
#    - OCR_NEAR_GATE_ONLY: Only OCR vehicles near gate (configurable range)
#    - OCR_EVERY_N_FRAMES: Run OCR every N frames instead of every frame
#    - Narrower detection band for faster processing
#    - USE_ADAPTIVE_BAND: Expands detection area based on last plate positions
#
# 3. TRACKING CONSISTENCY:
#    - PlateMemory: Maintains tracker-to-plate associations
#    - Confidence-based plate attachment (requires N consecutive detections)
#    - Automatic plate reattachment when tracker reappears
#    - Lost tracker timeout management
#    - pending_in_cross: Explicit crossing state management (from per6)
#    - Stale tracker cleanup every 10 frames
#
# 4. CSV LOGGING:
#    - Fixed ByteTrack integration to ensure proper CSV writes
#    - Immediate flush on every write for data safety
#    - Deduplication with suffix_event_allowed
#
# FEATURES:
...
# SINGLE EVENT MODE: Track only ONE direction (IN or OUT)
# Set to "in", "out", or "both" (both enables IN and OUT events)
EVENT_MODE               = "in"   # Options: "in", "out", "both"
# =================================================================
...
GATE_CAPTURE_MARGIN_PX   = 140    # start capture window when within this many px of gate
DRAW_GATE                = True
...
MIN_EVENT_GAP_FRAMES_CAMERA = 3

# One-shot window (fallback in case sharpness never crosses threshold)
CAPTURE_WINDOW_FRAMES    = 8
CAPTURE_WINDOW_FRAMES_CAMERA = 2

# Plate capture quality guards (lowered for live camera motion)
MIN_PLATE_SIDE_PX        = 14
MIN_CAPTURE_SHARPNESS    = 18.0

# ===================== OCR OPTIMIZATION =====================
# Optimized OCR processing for better FPS
# Only run OCR on frames near the gate crossing
OCR_NEAR_GATE_ONLY       = True   # Only OCR when vehicle is near gate
OCR_GATE_RANGE_PX        = 200    # OCR range from gate line (pixels)
OCR_EVERY_N_FRAMES       = 2      # Run OCR every N frames (reduces CPU load)
USE_ADAPTIVE_BAND        = True   # Expand detection band based on last seen plates
ADAPTIVE_BAND_MARGIN     = 40     # Margin around last detection (pixels)
# ==============================================================
...
# Camera-specific tweak: detection throttle when using live feed
INSIDE_PROCESS_EVERY_N_CAMERA = 2
...
# Improved tracking with plate memory
TRACKER_PLATE_MEMORY     = True   # Remember plates attached to tracker IDs
PLATE_CONFIDENCE_FRAMES  = 3      # Frames needed to confirm plate association
TRACKER_LOST_TIMEOUT     = 60     # Frames to remember lost tracker's plate
# =================================================================
...
def enrich_plate_fields(store, text):
    """Merge parsed plate fields into the provided store dict."""
    if not text:
        return
    region_class, suffix, kana, city, engine_size = parse_plate_fields(text)
    if region_class and not store.get("region_class"):
        store["region_class"] = region_class
    if suffix and not store.get("suffix"):
        store["suffix"] = suffix
    if kana and not store.get("kana"):
        store["kana"] = kana
    if city and not store.get("city"):
        store["city"] = city
    if engine_size and not store.get("engine_size"):
        store["engine_size"] = engine_size
    raw = store.get("raw_text")
    if text and (not raw or len(text) > len(raw)):
        store["raw_text"] = text
...
def suffix_event_allowed(history, suffix, direction, frame_idx, gap_frames):
    if not suffix:
        return True
    last_dir, last_frame = history.get(suffix, (None, -10**9))
    if last_dir == direction and (frame_idx - last_frame) < gap_frames:
        return False
    history[suffix] = (direction, frame_idx)
    return True
...
# ========== Plate Memory System ==========
class PlateMemory:
    """Maintains persistent tracker-plate associations for improved consistency."""
    def __init__(self):
        self.tracker_plates = {}       # tracker_id -> {"plate": text, "confidence": count, "last_frame": idx}
        self.plate_trackers = {}       # plate_key -> tracker_id (reverse lookup)
        self.lost_trackers = {}        # tracker_id -> {"plate": text, "lost_frame": idx}

    def update_tracker_plate(self, tracker_id, plate_text, frame_idx):
        """Update or confirm plate association for tracker."""
        if not plate_text:
            return

        if tracker_id not in self.tracker_plates:
            self.tracker_plates[tracker_id] = {
                "plate": plate_text,
                "confidence": 1,
                "last_frame": frame_idx
            }
        else:
            current = self.tracker_plates[tracker_id]
            if current["plate"] == plate_text:
                # Same plate, increase confidence
                current["confidence"] = min(current["confidence"] + 1, PLATE_CONFIDENCE_FRAMES + 5)
            else:
                # Different plate detected
                if current["confidence"] <= 1:
                    # Low confidence, replace
                    current["plate"] = plate_text
                    current["confidence"] = 1
                # else: keep existing high-confidence plate
            current["last_frame"] = frame_idx

        # Update reverse lookup
        pk = plate_key(plate_text)
        if pk:
            self.plate_trackers[pk] = tracker_id

    def get_tracker_plate(self, tracker_id):
        """Get confirmed plate for tracker (only if confidence is high enough)."""
        if tracker_id in self.tracker_plates:
            data = self.tracker_plates[tracker_id]
            if data["confidence"] >= PLATE_CONFIDENCE_FRAMES:
                return data["plate"]
        return None

    def get_plate_tracker(self, plate_text):
        """Get tracker_id associated with this plate."""
        pk = plate_key(plate_text)
        if pk and pk in self.plate_trackers:
            return self.plate_trackers[pk]
        return None

    def mark_tracker_lost(self, tracker_id, frame_idx):
        """Mark tracker as lost but remember its plate temporarily."""
        if tracker_id in self.tracker_plates:
            plate = self.tracker_plates[tracker_id]["plate"]
            self.lost_trackers[tracker_id] = {
                "plate": plate,
                "lost_frame": frame_idx
            }

    def cleanup_old_trackers(self, frame_idx):
        """Remove old lost trackers that have exceeded timeout."""
        to_remove = []
        for tid, data in self.lost_trackers.items():
            if frame_idx - data["lost_frame"] > TRACKER_LOST_TIMEOUT:
                to_remove.append(tid)

        for tid in to_remove:
            self.lost_trackers.pop(tid, None)
            if tid in self.tracker_plates:
                plate = self.tracker_plates[tid]["plate"]
                pk = plate_key(plate)
                if pk and self.plate_trackers.get(pk) == tid:
                    self.plate_trackers.pop(pk, None)
                self.tracker_plates.pop(tid, None)

    def clear_tracker(self, tracker_id):
        """Completely remove tracker from memory."""
        if tracker_id in self.tracker_plates:
            plate = self.tracker_plates[tracker_id]["plate"]
            pk = plate_key(plate)
            if pk and self.plate_trackers.get(pk) == tracker_id:
                self.plate_trackers.pop(pk, None)
            self.tracker_plates.pop(tracker_id, None)
        self.lost_trackers.pop(tracker_id, None)
# ==========================================
...
def main():
    ...
    # Initialize plate memory system
    plate_memory = PlateMemory() if TRACKER_PLATE_MEMORY else None
    ...
    # Adaptive detection band tracking (from per6)
    last_seen_frame = {}
    latest_det = {}
    pending_in_cross = set()  # Track vehicles that crossed into zone
    ...
    plate_enrichment = defaultdict(_new_enrichment_bucket)
    captured_in = set()
    suffix_event_history = {}

    inside_process_every_n = INSIDE_PROCESS_EVERY_N_CAMERA if using_camera else INSIDE_PROCESS_EVERY_N

    capture_window_frames = CAPTURE_WINDOW_FRAMES_CAMERA if using_camera else CAPTURE_WINDOW_FRAMES
    event_gap_frames = MIN_EVENT_GAP_FRAMES_CAMERA if using_camera else MIN_EVENT_GAP_FRAMES
    ...
    try:
        while True:
            ...
            if have_locked_inside and (frame_idx % max(1, inside_process_every_n) != 0):
            ...
            # Cleanup old tracker memory periodically
            if plate_memory and (frame_idx % 100 == 0):
                plate_memory.cleanup_old_trackers(frame_idx)

            # ========== OPTIMIZED OCR PROCESSING ==========
            # Only run OCR every N frames for better FPS
            run_ocr_this_frame = (frame_idx % OCR_EVERY_N_FRAMES == 0)
            ...
            if do_heavy_this_frame:
                ...
                if use_roi:
                    ...
                    # OPTIMIZATION: Narrow OCR band around gate for better performance
                    if OCR_NEAR_GATE_ONLY:
                        band_w = max(160, min(int(W * GATE_DET_BAND_RATIO), OCR_GATE_RANGE_PX * 2))
                    else:
                    ...
                    # ========== ADAPTIVE DETECTION BAND (from per6) ==========
                    # Expand detection area based on where plates were last seen
                    if USE_ADAPTIVE_BAND and latest_det:
                        min_x = d_rx
                        max_x = d_rx + d_rw
                        for det in latest_det.values():
                            ...
                            if not xy:
                                continue
                            x1, y1, x2, y2 = xy
                            min_x = min(min_x, max(0.0, x1 - ADAPTIVE_BAND_MARGIN))
                            max_x = max(max_x, min(float(W), x2 + ADAPTIVE_BAND_MARGIN))
            ...
            if do_heavy_this_frame:
                for idx, (det, text) in enumerate(zip(lp_dets_full, lp_texts)):
                    ...
                    # ========== PLATE MEMORY INTEGRATION ==========
                    # Update plate memory with new detections
                    if plate_memory and text:
                        plate_memory.update_tracker_plate(oid, text, frame_idx)

                    # Use confirmed plate from memory if available
                    confirmed_plate = plate_memory.get_tracker_plate(oid) if plate_memory else None
                    if confirmed_plate:
                        text = confirmed_plate  # Use high-confidence plate
                    # ==============================================

                    enrich_plate_fields(plate_enrichment[oid], text)

                    # Update tracking metadata (from per6)
                    ...
                    if oid not in prev_inside:
                        ...
                        if is_in:
                            ...
                            pending_in_cross.add(oid)  # Mark as needing IN event
                        continue

                    last_inside = prev_inside.get(oid, False)

                    # Detect crossing events (from per6)
                    if (not last_inside) and is_in and inside_count.get(oid, 0) + 1 >= ENTER_STABLE_FRAMES:
                        ...
                        if oid not in best_text:
                            best_text[oid] = ""
                        best_det[oid] = det
                    elif last_inside and (not is_in) and outside_count.get(oid, 0) + 1 >= EXIT_STABLE_FRAMES:
                    ...
                    last_event_frame = last_event_f.get(oid, -10**9)
                    ...
                    if crossed_in_now and (frame_idx - last_event_frame >= event_gap_frames):
                        ...
                        # ========== EVENT MODE FILTER: Skip if not tracking IN events ==========
                        if EVENT_MODE == "out":
                            # Only track OUT events, skip IN processing
                            crossed_in_now = False
                            continue
                        # ========================================================================
                        ...
                        if oid in captured_in:
                            ...
                            if slot_to_free is None:
                                ...
                                if slot_to_free is not None:
                                    ...
                                    fields = plate_enrichment.get(oid, {})
                                    region_class = fields.get("region_class") or region_class
                                    suf_out = fields.get("suffix") or suf_out
                                    kana = fields.get("kana") or kana
                                    city = fields.get("city") or city
                                    engine_size = fields.get("engine_size") or engine_size
                                    if fields.get("raw_text"):
                                        chosen_text = fields.get("raw_text")
                                    object_id = f"{slot_to_free:02d}"
                                    allowed_out = suffix_event_allowed(suffix_event_history, suf_out, "out", frame_idx, event_gap_frames)
                                    if allowed_out:
                            ...
                            plate_enrichment.pop(oid, None)
                            # Clear plate memory for this tracker
                            if plate_memory:
                                plate_memory.clear_tracker(oid)
                        ...
                        plate_enrichment[oid] = _new_enrichment_bucket()
                        enrich_plate_fields(plate_enrichment[oid], text)
                    else:
                        crossed_in_now = False

                    if crossed_out_now and (frame_idx - last_event_frame >= event_gap_frames):
                        if PRINT_GATE_CROSSINGS:
                            print(f"[GATE] tracker_id={oid} crossed → OUT, was_captured={'YES' if oid in captured_in else 'NO'}, cx={cx:.0f}, gate_right={gate_right}")

                        # ========== EVENT MODE FILTER: Skip if not tracking OUT events ==========
                        if EVENT_MODE == "in":
                            # Only track IN events, skip OUT processing
                            crossed_out_now = False
                            continue
                        # ========================================================================
                    ...
                    # one-shot capture maintenance (check pending_in_cross too)
                    if is_in and (oid not in captured_in or oid in pending_in_cross):
                        if capture_open.get(oid, 0) > 0:
                            ...
                            if min_side < MIN_PLATE_SIDE_PX:
                                capture_open[oid] -= 1
                                continue

                            sharp = variance_of_laplacian(plate_img)
                            if sharp < MIN_CAPTURE_SHARPNESS:
                                capture_open[oid] -= 1
                                continue
                            ...
                            # keep best
                            if score > best_score.get(oid, 0.0):
                        ...
                        # when window closes, log IN once (only if in pending_in_cross)
                        if (oid in pending_in_cross) and capture_open.get(oid, 0) == 0 and inside_count[oid] >= ENTER_STABLE_FRAMES:
                            # ========== EVENT MODE FILTER: Skip IN if only tracking OUT ==========
                            if EVENT_MODE == "out":
                                capture_open[oid] = -1  # Lock without logging
                                pending_in_cross.discard(oid)
                                continue
                            # ======================================================================
                            ...
                            if chosen_slot is not None:
                                fields = plate_enrichment.get(oid, {})
                                if fields.get("raw_text") and not chosen_text:
                                    chosen_text = fields.get("raw_text")
                                # Merge enriched fields into parsed
                                region_class = fields.get("region_class") or region_class
                                suf = fields.get("suffix") or suf
                                kana = fields.get("kana") or kana
                                city = fields.get("city") or city
                                engine_size = fields.get("engine_size") or engine_size
                                ...
                                allowed_in = suffix_event_allowed(suffix_event_history, suf, "in", frame_idx, event_gap_frames)
                                if allowed_in:
                                    write_row_flush(csv_writer, csv_file,
                    ...
                    if (not handled_exit_via_cross) and prev_inside[oid] and (not is_in) and outside_count[oid] >= EXIT_STABLE_FRAMES:
                        # ========== EVENT MODE FILTER: Skip OUT if only tracking IN ==========
                        if EVENT_MODE == "in":
                            # Only track IN events, skip fallback OUT processing
                            prev_inside[oid] = is_in
                            continue
                        # ======================================================================

                        if (frame_idx - last_event_f.get(oid, -10**9) >= event_gap_frames) and (oid in captured_in):
                            pk_now = active_id_to_plate.get(oid)
                            slot_to_free = None
                            if pk_now and pk_now in active_plate_to_slot:
                                slot_to_free = active_plate_to_slot.pop(pk_now, None)
                            ...
                            plate_enrichment.pop(oid, None)
                            pending_in_cross.discard(oid)
                            # Also clean up tracking metadata
                            latest_det.pop(oid, None)
                            last_seen_frame.pop(oid, None)
                            # Clear plate memory for this tracker
                            if plate_memory:
                                plate_memory.clear_tracker(oid)
            ...
            # ========== STALE TRACKER CLEANUP (from per6) ==========
            # Clean up trackers that haven't been seen in a while
            if frame_idx % 10 == 0:
                TRACK_STALE_FORGET = 90  # frames
            ...
            if SHOW_FPS:
                ...
                event_mode_display = EVENT_MODE.upper()
                event_color = (100, 255, 100) if EVENT_MODE == "both" else (255, 200, 100)
                cv2.putText(frame, f"Event: {event_mode_display}", (10, 72),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.55, event_color, 2, cv2.LINE_AA)

                # Plate memory status
                if plate_memory and TRACKER_PLATE_MEMORY:
                    tracked_count = len(plate_memory.tracker_plates)
                    cv2.putText(frame, f"Tracked plates: {tracked_count}", (10, 96),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 255), 2, cv2.LINE_AA)
                ...
                for oid in oids_to_exit:
                    ...
                    if slot_to_free is not None:
                        ...
                        if fields.get("raw_text"):
                            chosen_text = fields.get("raw_text")
                        object_id = f"{slot_to_free:02d}"
                        if suffix_event_allowed(suffix_event_history, suf_out, "out", frame_idx, event_gap_frames):
                            write_row_flush(csv_writer, csv_file,
                                            [ts, object_id, "car", "out", city or "", engine_size or "", kana or "", suf_out or ""])
                        last_event_f[oid] = frame_idx
                        slot_pool.release(slot_to_free)
