...
DRAW_GATE                = True
GATE_INSIDE_IS_RIGHT     = False  # <<< inside is LEFT of the line (right->left = IN), toggle with [o]
...
def main():
    ...
    # --- NEW: parent-child session by 4-digit suffix ---
    plate_sessions = {}  # suffix -> {"slot": int, "in_ts": str}

    # --- Direction helper (store previous cx) ---
    prev_cx = {}
        ...
        while True:
            ...
            if do_heavy_this_frame:
                for det, text in zip(lp_dets_full, lp_texts):
                    ...
                    # --- Direction inference: store last cx, see which side we came from
                    last_cx = prev_cx.get(oid)
                    prev_cx[oid] = cx
                    ...
                    if is_in and (oid not in captured_in):
                        ...
                        if capture_open.get(oid, 0) == 0 and inside_count[oid] >= ENTER_STABLE_FRAMES:
                            ...
                            if pk and pk in active_plate_to_slot:
                                ...
                                # --- record parent session by suffix
                                if suf: plate_sessions[suf] = {"slot": chosen_slot, "in_ts": ts}
                                prev_inside[oid] = is_in
                                continue
                    ...
                    if prev_inside[oid] and (not is_in) and outside_count[oid] >= EXIT_STABLE_FRAMES:
                        if frame_idx - last_event_f.get(oid, -10**9) >= MIN_EVENT_GAP_FRAMES:
                            ...
                            if slot_to_free is not None:
                                ...
                                # --- complete child session by suffix ---
                                if suf_out and suf_out in plate_sessions:
                                    # (No CSV change; pairing is internal)
                                    plate_sessions.pop(suf_out, None)
