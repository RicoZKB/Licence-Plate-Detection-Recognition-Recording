        ...
        while True:
            ...
            if do_heavy_this_frame:
                for det, text in zip(lp_dets_full, lp_texts):
                    ...
                    # NEW: Detect gate crossing and reopen capture window
                    if crossed and crossing_direction:
                        ...
                        if last_crossing_direction.get(oid) != crossing_direction:
                            ...
                            # UNLOCK the capture window (remove -1 lock) for fresh OCR
                            ...
                            # Determine event type based on crossing direction and gate orientation
                            # (This will be used later in the IN/OUT event logging)
                            if gate_right:
                                event_is_entry = (crossing_direction == 'left_to_right')
                            else:
                                event_is_entry = (crossing_direction == 'right_to_left')

                            # Note: The actual event logging happens below in the existing code
                            # Continue to OCR processing below
