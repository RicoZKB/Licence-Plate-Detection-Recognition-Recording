# -*- coding: utf-8 -*-
# region07.py — ROI-cropped, one-shot plate capture, zero work outside ROI
# - Detector/OCR runs ONLY inside the yellow ROI (outside = no heavy work)
# - On first clear, valid frame after ENTER, capture once and stop OCR
# - ROI is resizable (W/A/S/D move, [ ] resize, r reset)
# - Joins OCR lines into one string like: "神戸567 20-56 ふ"
...
# ROI is RESIZABLE now
LOCK_ROI                 = False

# Stability & gaps
ENTER_STABLE_FRAMES      = 1     # capture early
EXIT_STABLE_FRAMES       = 4
MIN_EVENT_GAP_FRAMES     = 10

# One-shot window as fallback if sharpness never crosses threshold
CAPTURE_WINDOW_FRAMES    = 6

# While an object is inside AFTER we captured it, throttle heavy work
...
# Performance toggles
USE_CAR_DETECTOR         = False  # leave False for lowest load
WRITE_VIDEO              = False
DRAW_BBOXES              = True
SHOW_FPS                 = True
TARGET_WIDTH             = 640
...
# ---------- text helpers ----------
def norm_text(t):
    # Join list/tuple into a single string
...
def main():
    ...
    try:
        while True:
            ...
            # --- ONLY work inside ROI (outside = no heavy work) ---
            rx, ry, rw, rh = clamp_region(rx, ry, rw, rh, W, H)
            roi = crop_frame(frame, rx, ry, rw, rh)

            # throttle once captured to keep FPS high while still inside
            ...
            if do_heavy_this_frame:
                ...
                roi_dets = all_lp_dets[0]
                # join lines -> "神戸567 20-56 ふ"
                lp_texts = [norm_text(t) for t in all_lp_texts[0]]
            ...
            # Process detections (only when we ran them)
            if do_heavy_this_frame:
                for det, text in zip(lp_dets_full, lp_texts):
                    ...
                    if is_in and (oid not in captured_in):
                        if capture_open.get(oid, 0) > 0:
                            ...
                            # Early accept at first inside OR sharp enough, but require suffix
                            is_first_inside = (inside_count.get(oid,0) == 1)
                            if plate_suffix(text or "") and (sharp >= MIN_SHARPNESS_LOCK or is_first_inside):
                                best_score[oid] = score
                                best_text[oid]  = text
                                best_det[oid]   = det
                                capture_open[oid] = 0
                            else:
                                # keep best as fallback only if suffix present
                                if plate_suffix(text or "") and score > best_score.get(oid, 0.0):
                                    best_score[oid] = score
                                    best_text[oid]  = text
                                    best_det[oid]   = det
                                capture_open[oid] -= 1
                        ...
                        if capture_open.get(oid, 0) == 0 and inside_count[oid] >= ENTER_STABLE_FRAMES:
                            chosen_text = norm_text(best_text.get(oid,"") or text or "")
                            ...
                            if chosen_slot is not None:
                                ...
                                capture_open[oid] = -1  # LOCK: skip future OCR until exit
                                if DRAW_BBOXES:
            ...
            # Display / write
            cv2.imshow("Parking (ROI one-shot)", frame)
            if out is not None and WRITE_VIDEO: out.write(frame)

            # ROI keyboard control (enabled since LOCK_ROI=False)
